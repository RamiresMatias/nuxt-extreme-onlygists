<template>
  <div class="w-full flex flex-col justify-center gap-2 my-5">
    ...
    <div v-html="description" class="font-[Inter] text-gray-700 font-regular text-base md:text-xl"></div>

    <p class="font-[Inter] font-regular text-gray-500 text-sm md:text-base">
      <i class="pi pi-star"></i>-
      by <NuxtLink class="text-primary-500" :to="`/${props.author}`">@{{ props.author }}</NuxtLink>
    </p>
  </div>
</template>
...
const props = withDefaults(
  ...
  title: 'Currency.format.ts',
  lang: 'typescript',
  description: 'Função em typescript para fazer formatação de moedas',
  author: 'ramiresmatias'
})
